...
class HistoryEncoder(torch.nn.Module):
    def __init__(self, input_size: int, hidden_size: int, cnn_filters: int, cnn_kernels: list[int], num_lstm: int, dropout: float):
        super().__init__()
        
        if not cnn_kernels:
            raise ValueError("The cnn_kernels list must not be empty!")

        self._relu = torch.nn.ReLU()
        self._cnns = torch.nn.ModuleList()
        self._bnns = torch.nn.ModuleList()
        ...
        for kernel in cnn_kernels:
            ...
            self._bnns.append(torch.nn.BatchNorm1d(num_features=cnn_filters))
        ...
        self._lstm = torch.nn.LSTM(
            ...
            batch_first=True, 
            bidirectional=True,
            dropout=dropout if num_lstm > 1 else 0.0
        )
    ...
    def forward(self, x):
        ...
        for i, (cnn, bn) in enumerate(zip(self._cnns, self._bnns)):
            if i == 0:
...
class FutureEncoder(torch.nn.Module):
    def __init__(self, input_size: int, hidden_size: int, cnn_filters: int, cnn_kernels: list[int], num_lstm: int, dropout: float):
        ...
        if not cnn_kernels:
            raise ValueError("Seznam cnn_kernels nesmí být prázdný!")
...
class Decoder(torch.nn.Module):
    ...
    def decoder_prediction(self, encoded: torch.Tensor, encoded_future: torch.Tensor, previous_power: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(horizon):
            ...
            if i < horizon - 1:
                own_predicted_fve = predictions 
                meteo_now = encoded_future[:, i + 1, :] # Update obohacených meteo dat
...
class Model(torch.nn.Module): 
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        history_kernels = []
        i = 0
        while hasattr(args, f"past_kernel_L{i}"):
            history_kernels.append(getattr(args, f"past_kernel_L{i}"))
            i += 1
        ...
        self.encoder = HistoryEncoder(
            ...
            cnn_kernels=history_kernels, 
            num_lstm=args.past_num_lstm,  
        ...
        self.decoder = Decoder(
            ...
            hidden_size=args.past_hidden_size * 2, 
